...
const Deposit = () => {
    ...
    axios({
        method:'put',
        url:'http://localhost:7000/api/deposit',
        data:{
            deposit:JSON.parse(localStorage.getItem('amount'))
        },
        headers:{
            'Content-Type':'application/json',
            Authorization:localStorage.getItem('token')
    ...
    return(
        <div>
            ...
            <div className='container p-2 border mt-5' style={{borderRadius:'20px'}}>
                <h3 className='mt-2 mb-4'>Deposit Money into Your Account!</h3>
                ...
                <div>
                {localStorage.getItem('amount') ? 
                <button className='btn btn-info btn-sm mt-3' onClick={handleSubmit()}>Deposit</button>
                :
                <button className='btn btn-info btn-sm mt-3' onClick={()=>handleSubmit()}>Deposit</button>
                }
                </div>
