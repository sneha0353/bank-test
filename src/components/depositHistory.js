...
const Deposithistory = () => {
    ...
    const WithrawDate = () => {
        fetch("http://localhost:7000/api/getdepositdate",{
            method:"post",
            headers:{
    ...
    useEffect(()=>{
        fetch("http://localhost:7000/api/getdeposithistory",{
            method:'get',
            headers:{
                Authorization:localStorage.getItem('token')
            }
    ...
    return(
        <div>
            ...
            <div>
                <div className="row">
                    <div className="col-6">
                        <h2 className="text-info mt-2">Deposit History</h2>
                        {whistory ?
                            <div>
                                {flag ? 
                                    <div>
                                            ...
                                            return(
                                                <div class="card m-2" style={{width: "100%",borderRadius:'20px'}}>
                                                    <div class="card-body">
                                                        ...
                                                        <h5 class="card-title">Amount Deposit : <span className="text-danger">{item.damount}</span></h5>
                                                        <h5 class="card-title">Deposit Date : <span className="text-danger">{item.date.slice(0,10)}</span></h5>
                                                    </div>
                                                </div>
                                            )
                                        })}
